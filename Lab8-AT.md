## Практическое задание №8 по дисциплине "Компьютерные технологии на транспорте", ВШЭКН ЮУрГУ, 2020/2021 учебный год
...
Для создания объектов базы данных используется оператор CREATE. Перед тем, как создать свою первую базу данных "Преподаватели", потренируемся в создании и удалении тестовой базы данных. Убедитесь, что вы включили запись ваших в команд во внешний log-файл и набрали команду SELECT NOW();.
...
+ Заполним таблицу Lecturers. Ниже приведены примеры заполнения таблицы, вам необходимо заполнить ее своими данными из первой практической работы. Для добавления записей в таблицу используется оператор INSERT. Сначала внесем в таблицу одну запись, причем воспользуемся тем, что в поле LecturerID установлена автонумерация.
  ...
  Аналогично вышеуказанным примерам, заполните вашу таблицу своими данными, которые вы использовали при выполнении практического задания 7. При заполнении таблицы проверьте реакцию системы на ввод неправильных данных, например попробуйте для поля Position указать значение 'Лаборант'.
...
+ Наберите команду SELECT NOW(); для вывода текущего времени, завершите запись команд в файл и отправьте ваш файл "P8_Фамилия.log" на проверку (в элементе курса "Сдать на проверку Практическое задание 8").
